...
class Game:
    ...
    def handle_keypress(self, event):
        # Vérifiez si la touche pressée est la flèche droite avec event.key == pygame.K_RIGHT
            # Vérifiez si Pac-Man peut se déplacer à droite sans entrer en collision avec self.check_collision((1, 0))
                # Si le déplacement est possible, définissez la nouvelle direction de Pac-Man vers la droite avec self.pacman.set_direction((1, 0))
        if event.key == pygame.K_RIGHT:
            if self.check_collision((1, 0)): # Pac-Man n'entre pas en collision vers la droite
                self.pacman.set_direction((1, 0))

        # Vérifiez si la touche pressée est la flèche gauche
        ...
        # Vérifiez si la touche pressée est la flèche vers le haut
        ...
        # Vérifiez si la touche pressée est la flèche vers le bas
    ...
    def check_collision(self, direction):
        '''Returns True if there's no collision and False if it will hit a wall.'''

        # Extraire les coordonnées de déplacement de la direction (dx, dy)
        dx, dy = direction

        # Calculer la nouvelle position de Pac-Man après le déplacement (new_x, new_y) avec la formule new_x = self.pacman.x + dx
        new_x = self.pacman.x + dx
        new_y = self.pacman.y + dy

        # Vérifier si la nouvelle position est un chemin valide ou un mur
    ...
    def check_score(self):
        # Vérifier si la position actuelle de Pac-Man (en coordonnées de grille) correspond à une position de pièce en utilisant (self.pacman.x, self.pacman.y)
        position = (self.pacman.x, self.pacman.y)

            # Si Pac-Man est sur une pièce, la retirer de la liste des pièces restantes à collecter 
        if position in self.coins:
            self.coins.remove(position)

            # Ajouter des points au score du joueur pour la pièce collectée (par exemple, 10 points)
            self.score += 10
    ...
    def check_special_coins(self):
        # Vérifier si la position actuelle de Pac-Man (en coordonnées de grille) correspond à une position de pièce spéciale
        position = (self.pacman.x, self.pacman.y)

        if position in self.special_coins:
            # Si Pac-Man est sur une pièce spéciale, retirer cette pièce spéciale de la liste
            self.special_coins.remove(position)

            # Ajouter des points au score du joueur pour la pièce spéciale collectée
            self.score += 15

            # Activer le mode "manger" en appelant la méthode appropriée pour activer le mode "manger" des fantômes avec self.activate_eat_mode()
