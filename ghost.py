...
class Ghost:
    ...
    def move(self):
        ...
        if not self.dead:
            ...
            if self.check_collision(next_rect) == False:
                # Si aucune collision n'est détectée, mettre à jour la position du fantôme
    ...
    def change_direction(self):
        # Créer une liste de toutes les directions possibles pour le fantôme (gauche, droite, haut, bas)
        moves = [Direction.UP, Direction.DOWN, Direction.LEFT, Direction.RIGHT]

        # Mélanger aléatoirement les directions pour simuler un choix aléatoire avec `random.shuffle()`
        ...
        for move in moves:
            ...
            next_x = self.pos[0] + move[0]
            next_y = self.pos[1] + move[1]
            
            # Créer un rectangle représentant cette nouvelle position
            next_rect = pygame.Rect(next_x, next_y, GHOST_SIZE[0], GHOST_SIZE[1])
        
            # Vérifier si cette direction entraîne une collision avec un mur en utilisant `self.check_collision()`
            if self.check_collision(next_rect) == False:
                # Si aucune collision n'est détectée, définir cette direction comme la nouvelle direction du fantôme avec `self.set_direction()` et sortir de la boucle
                self.direction = move
